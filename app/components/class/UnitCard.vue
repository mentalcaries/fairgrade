...
import type { Unit, Student, Instructor } from '~/types'
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card'
import { Button } from '@/components/ui/button'
import { Users, CheckCircle2, AlertTriangle, Trash2 } from 'lucide-vue-next'
...
const gradingStatus = computed(() => {
  ...
  if (gradedStudents.value === unitStudents.value.length) {
    return { type: 'complete', label: `${gradedStudents.value}/${unitStudents.value.length} graded` }
  }
  return { type: 'incomplete', label: `${gradedStudents.value}/${unitStudents.value.length} graded` }
})
</script>

<template>
  <Card class="bg-card border-border hover:shadow-md transition-shadow relative group">
    <button
      class="absolute top-3 right-3 p-1.5 rounded-md text-red-500 hover:text-red-600 hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-opacity z-10"
      title="Delete unit"
